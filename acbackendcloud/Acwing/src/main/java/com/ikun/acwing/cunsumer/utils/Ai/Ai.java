...
public class Ai {
    ...
    private  static boolean[][][] wins;// 赢法数组，记录了每一种赢法
    public Integer[][] chessBoard;// 当前棋盘，NONE代表空位，MAX代表AI，MIN代表对手
    ...
    public Ai(int row, int column,Integer[][] board,Stack<int[]> s) {
        this.row = row;
        this.column = column;
        isEnded = false;
        ...
        for(int[] t:s){
            stack.push(new Point(t[0],t[1],t[2]));
        }
        // 初始化wins赢法数组
        // 横向
        for (int i = 0; i < row; i++) {
    ...
    public Map<String,String> AiNextSteps(){
        Map<String,String> map = new HashMap<>();
        EvaluetePoint p = max(4,INFINITY);
        map.put("x",Integer.valueOf(p.getX()).toString());
        map.put("y",Integer.valueOf(p.getY()).toString());
        return map;
    }
